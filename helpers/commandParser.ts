...
import { authAPI, blogAPI, contactAPI, commentsAPI, reactionsAPI, type Article } from '@/services/api';
import { 
  formatArticleList, 
  formatArticleDetail, 
  formatSearchResults, 
    ...
    case 'contact':
      ...
      if (args[0] === '-github') {
        if (args[1] === '-copy') {
          ...
          if (args.length < 3) {
            return {
              output: 'Usage: contact -github -clone <project-name | project-number>\nExample: contact -github -clone 1',
              type: 'error'
            };
          }
          return handleCloneCommand(args.slice(2));
        }
        return {
          output: `GitHub: ${contactData.social.github}\nTip: Use 'contact -github -copy' to copy, 'contact -github -open' to open, 'contact -github -repos' to see repositories, or 'contact -github -clone <project>' to copy clone command`,
          type: 'info'
        };
      }
      ...
      return {
        output: 'Usage: contact [-email -copy | -linkedin -copy/-open | -github -copy/-open/-repos/-clone <project> | -qrz -copy/-open]\nExample: contact -github -repos',
        type: 'error'
      };
...
const formatHelp = (userRole?: string): string => {
  ...
  const categories = ['navigation', 'content', 'general', 'auth'];
...
${'='.repeat(60)}\n\n`;
  ...
  projectsData.projects.forEach((project, index) => {
    ...
    output += `   📁 ${project.gitUrl || project.link}\n`;
    output += `   Status: ${project.status}\n`;
    output += `   Tech: ${project.technologies.join(', ')}\n\n`;
  });

  output += `\nTip: Use 'contact -github -clone <number>' to copy the git clone URL\n`;
  output += `Example: contact -github -clone 1\n`;

  return output;
};

const handleCloneCommand = (args: string[]): { output: string; type: 'success' | 'error' | 'info' } => {
  const input = args.join(' ');
  let project = null;

  // Try to find by number first
  const num = parseInt(input);
  if (!isNaN(num) && num > 0 && num <= projectsData.projects.length) {
    project = projectsData.projects[num - 1];
  } else {
    // Try to find by name (case insensitive)
    const lowerInput = input.toLowerCase();
    project = projectsData.projects.find(p => 
  ...
  if (!project) {
    return {
      output: `Project not found: "${input}"\nUse 'repos' to see all available repositories.`,
      type: 'error'
    };
  }

  const gitUrl = project.gitUrl || project.link;
  const cloneCommand = `git clone ${gitUrl}`;
  
  return {
    output: '__COPY__' + cloneCommand,
    type: 'success'
  };
};
...
const handleAdminHelp = (userRole: string | undefined): { output: string; type: 'success' | 'error' | 'info' } => {
  ...
  output += '  forgot-password      - Request password reset email\n';
  output += '  reset-password       - Reset password with token from email\n';
...
export const executeAsyncCommand = async (asyncOutput: string): Promise<{ output: string; type: 'success' | 'error' | 'info' }> => {
  ...
  try {
    ...
    if (action === 'forgot-password') {
      ...
      // Check if we got dev mode response with token
      if ((result as any).data?.resetToken) {
        return {
          output: `✅ Password reset token generated! (Development Mode - Email not configured)\n\n📧 Email: ${email}\n🔑 Reset Token: ${(result as any).data.resetToken}\n\nUse this command to reset:\nreset-password ${(result as any).data.resetToken} <new-password>\n\nExample:\nreset-password ${(result as any).data.resetToken} myNewPassword123`,
          type: 'success'
        };
      }
      
      return {
        output: `✅ Password reset email sent!\n\nIf an account with email ${email} exists, you will receive a password reset link shortly.\n\nCheck your email and use: reset-password <token> <new-password>`,
        type: 'success'
      };
    }
